...
string execute(string command, string text=NULL, long long exp=0, long long N=0) {
  ...
  //cout << &command << " " << &text << "|" << command <<"|" << text << endl;
	if (cmd == "create") {
		...
		md = m;
		d = (Mint(1) / e)();
    sprintf(buffer, "{\"p\": \"%lld\", \"q\": \"%lld\", \"n\": \"%lld\", \"m\": \"%lld\", \"e\": \"%lld\", \"d\": \"%lld\"}", p, q, n, m, e, d);
    //cout << d <<" " << e <<" " << m << " " << n <<" " << p <<" " << q <<" " << Mint(d) * Mint(e) <<" " << buffer << endl;
		return buffer;
	}
	else if (cmd == "encrypt") {
		input = (string) text;
		...
		for (int i = 0; i < (int) input.size(); i += 7) {
			...
			now = power((Mint)now, e)();
			function<void(long long, int)> dfs = [&](long long x, int cnt) {
				if (cnt == 0) return ;
				dfs(x / 64, cnt - 1);
				res += base[x % 64];
			};
